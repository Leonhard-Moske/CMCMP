...
def diagonalizeT(T, beta):
    ...
    np.fill_diagonal(D, np.exp(np.multiply(eVa.real, -beta)))
...
T , v = wrapLancoz(csr_matrix(ham),Lx*Ly,m, phi)
...
for i in range(300):
    ...
    E.append(np.vdot(phiPrime, ham.dot(phiPrime)))
    E2.append(np.vdot(phiPrime, ham2.dot(phiPrime)))
    cV.append(beta**2*(np.vdot(phiPrime, ham2.dot(phiPrime)) - np.vdot(phiPrime, ham.dot(phiPrime))**2))
